...
func RegisterUser(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		fmt.Println("✅ Bound user:", user.Email, "Role:", user.Role, "Genres:", user.FavouriteGenres)
		...
		fmt.Println("✅ Validation passed for:", user.Email)
...
func LoginUser(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := c.ShouldBindJSON(&userLogin); err != nil {
			fmt.Println("❌ Login bind error:", err.Error())
			c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid input data"})
			return
		}
		...
		err := userCollection.FindOne(ctx, bson.D{{Key: "email", Value: userLogin.Email}}).Decode(&foundUser)
		if err != nil {
			fmt.Println("❌ User not found:", userLogin.Email)
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Invalid email or password"})
			return
		}
		...
		err = bcrypt.CompareHashAndPassword([]byte(foundUser.Password), []byte(userLogin.Password))
		if err != nil {
			fmt.Println("❌ Password mismatch for:", userLogin.Email)
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Invalid email or password"})
			return
		}
		...
		fmt.Println("✅ Login successful:", foundUser.Email)
		c.JSON(http.StatusOK, models.UserResponse{
			UserId:          foundUser.UserID,
			FirstName:       foundUser.FirstName,
			LastName:        foundUser.LastName,
...
func RefreshTokenHandler(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		fmt.Println("✅ Tokens refreshed for user:", claim.UserId)
		c.JSON(http.StatusOK, gin.H{"message": "Tokens refreshed"})
	}
}